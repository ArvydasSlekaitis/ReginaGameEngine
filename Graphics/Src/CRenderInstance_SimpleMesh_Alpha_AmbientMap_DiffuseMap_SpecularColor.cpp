///////////////////////////////////////////////////////////
//  CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor.cpp
...
CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::~CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor()
	{
	}

//*****************************************************************************

CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor(LPDIRECT3DDEVICE9 iDevice, LPD3DXMESH iMesh) : CRenderInstance_SimpleMesh(iDevice, matterialType, renderInstanceType, effectName, iMesh, instanceVertexElement, instanceDataSize)
  	{
	}

//*****************************************************************************

void CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::SetEffectParams(LPD3DXEFFECT& ioEffect, const vector<CRenderObject_SimpleMesh>& iRenderList)
	{
	...
	if(RenderType() == Regina::RenderType_Default )
		...
		if(directionalLight.size()==0)
			...
			assert(iRenderList.size() > 0 && L"CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::SetEffectParams - iRenderList.size() > 0");
    		CMatterial matterial = iRenderList[0].Matterial();

			string ambientMapName;
			matterial.GetProperty("ambientMap", ambientMapName);
			C2DTexture* ambientMap = TextureHandler->Get2D(ambientMapName);
			if(ambientMap!=NULL)
				{
				ioEffect->SetTexture( "ambientMap", ambientMap->Texture());
				}
...
*/
	...
	if(RenderType() == Regina::RenderType_Shadow )
		...
		{
		throw invalid_argument("CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::SetEffectParams - RenderType is invalid");
		}
	}

//*****************************************************************************

void CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::FillInstanceVB(LPDIRECT3DVERTEXBUFFER9& ioVertexBuffer, const vector<CRenderObject_SimpleMesh>& iRenderList)
	{
	...
	for(int i=0; i<iRenderList.size(); i++)
		...
		float alpha;
		iRenderList[i].Matterial().GetProperty("transparency", alpha);
		pVertices[i].r4=D3DXVECTOR4(alpha, 0, 0, 0);
...
const eMatterialType		CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::matterialType(MatterialType_Alpha_AmbientMap_DiffuseMap_SpecularColor);
const eRenderInstanceType	CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::renderInstanceType(RenderInstanceType_Alpha_AmbientMap_DiffuseMap_SpecularColor);
const string				CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::effectName("SimpleMesh_Instance_Alpha_AmbientMap_DiffuseMap_SpecularColor");
const unsigned				CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::instanceDataSize = sizeof(MATRIX_V4_V4_V4);
D3DVERTEXELEMENT9*			CRenderInstance_SimpleMesh_Alpha_AmbientMap_DiffuseMap_SpecularColor::instanceVertexElement(VE_MESH_POS_NORM_BINORM_TAN_UV_INSTANCE_WORLD);
