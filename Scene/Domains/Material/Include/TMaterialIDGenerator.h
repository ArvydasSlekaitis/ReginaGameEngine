/*
    TMaterialIDGenerator.h - Contains template TMaterialIDGenerator.
	Copyright 2011 Arvydas Ðlekaitis. All Rights Reserved.
*/


#ifndef REGINA_TMATERIALIDGENERATOR_H
#define REGINA_TMATERIALIDGENERATOR_H
...
/**
 * This  class is used to generate material ID.
 */
template <class T1> 
class TMaterialIDGenerator
{
private:
	/**
...
*/
template <class T1> 
unsigned TMaterialIDGenerator<T1>::Register(T1* iObject)
	{
...
*/
template <class T1> 
unsigned TMaterialIDGenerator<T1>::Register(T1* iObject, const unsigned& iPossibleID)
	{
	...
	if(*objectVariationList[iPossibleID].first->CompareMaterialID(*iObject) == true)
		{
...
*/
template <class T1> 
unsigned TMaterialIDGenerator<T1>::Update(T1* iObject, const unsigned& iCurID)
	{
	const unsigned position = GetObjectPosition(iObject);
...
*/
template <class T1> 
void TMaterialIDGenerator<T1>::Unregister(T1* iObject, const unsigned& iCurID)
	{
	DecreaseOccurance(iCurID, iObject);
	}
...
*/
template <class T1> 
void TMaterialIDGenerator<T1>::DecreaseOccurance(const unsigned& iPosition, T1* iObject)
	{ 
	assert(objectVariationList[iPosition].size()!=0 && L"TMaterialIDGenerator::DecreaseOccurance"); 
...
*/
template <class T1> 
unsigned TMaterialIDGenerator<T1>::GetObjectPosition(T1* iObject)
	{
	...
	for(unsigned i=0; i<variationListSize; i++)
		if(objectVariationList[i].size()!=0)
			if(iObject->CompareMaterialID(*(*objectVariationList[i].begin())) == true)
				return i;
...
*/
template <class T1> 
unsigned TMaterialIDGenerator<T1>::AddNewVariation(T1* iObject)
	{
	if(freeSlots.size() > 0)
		{
		const unsigned position = *freeSlots.begin();
		assert(objectVariationList[position].size() == 0 && L"TMaterialIDGenerator<T1>::AddNewVariation");
